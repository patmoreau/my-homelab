...
# Wait for Postgres to be ready. Under Podman the services share one network namespace,
# so postgres is reachable on 127.0.0.1 (there is no aardvark DNS for the name 'postgres').
...
psql -v ON_ERROR_STOP=1 -h 127.0.0.1 -U "$POSTGRES_USER" -d "$POSTGRES_APP_DB" <<-EOSQL
